...
export const RegisterPage: React.FC = () => {
  ...
  const [createdReceiverId, setCreatedReceiverId] = useState<string | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { receiverId } = await register(fullName, username, email, password);
      setCreatedReceiverId(receiverId);
    } catch (err: any) {
  ...
  const copyReceiverId = () => {
    if (createdReceiverId) {
      navigator.clipboard.writeText(createdReceiverId);
      setCopied(true);
